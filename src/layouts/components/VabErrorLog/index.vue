  ...
  <div v-if="errorLogs.length > 0">
    <el-badge :value="errorLogs.length" @click.native="dialogTableVisible = true">
      <el-button type="danger">
        <ht-icon :icon="'el-icon-cpu'" />
      </el-button>
    </el-badge>

    <el-dialog :visible.sync="dialogTableVisible" append-to-body width="70%" title="异常捕获(温馨提示：错误必须解决)">
      <el-table :data="errorLogs">
        ...
        <el-table-column width="380" label="操作">
          <template slot-scope="{ row }">
            <a v-for="(item, index) in searchList" :key="index" :href="item.url + decodeUnicode(row.err.message)" target="_blank">
              <el-button style="margin-left: 5px" type="primary">
                <ht-icon :icon="'el-icon-search'" />
                {{ item.title }}
              </el-button>
            </a>
          </template>
