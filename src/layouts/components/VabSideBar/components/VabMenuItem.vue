<template>
  <el-menu-item :index="handlePath(routeChildren.path)" @click="handleLink">
    <ht-icon v-if="routeChildren.meta.icon" :icon="routeChildren.meta.icon" class="vab-fas-icon" />
    ...
    handleLink() {
      ...
      if (target === "_blank") {
        if (isExternal(routePath)) {
          ...
          const routeData = this.$router.resolve(path.resolve(this.fullPath, routePath));
